...
const [imageViewVisible,setIsImageViewVisible]= React.useState(false);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.paddedScrollView} testID="cargoRequestDetailScrollView">
      <Spinner visible={  fetching  } textStyle={{ color: '#FFF' }} />
            ...
            <Card key={index}>
              <UserInfo>
                <TouchableOpacity 
                  ...
                  <UserImg source={ {uri :item.fromUser?.imageUrl ?
                   item.fromUser?.imageUrl : require('../../../../assets/avatar3.jpg')}} />
                </TouchableOpacity>
                <UserInfoText>
                  <TouchableOpacity
                    onPress={() => props.navigation.navigate('AppUserDetail', { entityId: item.fromUser.id, whoView: 'courier' })}>
...
const mapStateToProps = (state) => {
  return {
    cargoRequest: state.cargoRequests.cargoRequeswt,
