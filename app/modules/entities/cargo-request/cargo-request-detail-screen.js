...
function CargoRequestDetailScreen(props) {
  ...
  const confirmChange = () => {
    ...
    item2.cargoRequest = { id : cargoRequest.id};
    item2.fromUser = bidToChange.fromUser ;
    updateBid(item2);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.paddedScrollView} testID="cargoRequestDetailScrollView">
      ...
      <View style={{ textAlign: 'center', alignContent: 'center', alignItems: 'center' }}>
         {cargoRequest?.imageUrl&&( <PackageImg source={ {uri :cargoRequest?.imageUrl ? 
          cargoRequest?.imageUrl : require('../../../../assets/package.png') }} /> 
              )}
          </View>

        {account.id == cargoRequest.createBy.id ? (
          <ControlIcons style={{ width: '100%', textAlign: 'right', padding: 0 }}>
            <Ionicons name="trash-outline" size={30} color={'red'} onPress={() => setDeleteModalVisible(true)} style={{padding:5}}/>
